/*
	...
	<コピー元>
		ローカル上に存在するディレクトリへの相対パスまたは絶対パス。相対パスの場合、実行時のカレントからとする。
	<コピー先>
		ローカル上に存在する又は作成可能なディレクトリへの、相対パスまたは絶対パス。相対パスの場合、実行時のカレントからとする。
	<パスリストファイル>
		対象パスのリストを記述したファイル
		文字コード CP932(Shift_JIS) 改行コード CR-LF or LF のテキストファイルであること。
		対象パスを改行区切りで記述する。
		対象パス前後の不要な空白などや、空行を入れてはならない。但し EOF の直前は改行であっても良い。
		１件以上あること。
	対象パス
		ローカル上に存在するディレクトリまたはファイルへの、相対パスまたは絶対パス。相対パスの場合、コピー元からとする。
		コピー元の配下のパスでなければならない。また、コピー元自身を指定してはならない。

	/-C
		コピー前にコピー先をクリアしない。デフォルトではする。
	/-N
