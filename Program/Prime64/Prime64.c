...
//	errorCase(setvbuf(fp, NULL, _IOFBF, 64 * 1024 * 1024)); // ? é∏îs // old
...
static uint64 ToValue_Check(char *str)
{
	...
	errorCase_m(strcmp(str, tmp), "10êiêÆêîÇ≈ÇÕÇ»Ç¢ñîÇÕílàÊäO");
	memFree(tmp);
	return value;
}
static void Values_CheckRange(uint64 minval, uint64 maxval)
{
	errorCase_m(maxval < minval, "ç≈è¨ílÇ™ç≈ëÂílÇÊÇËëÂÇ´Ç¢");
}
