/*
	...
	最大値以下の素数を表示する。
	メモリが充分にあれば Prime2tox_memB.exe 31100000000 くらいまでいけるはず。
*/
...
int main(int argc, char **argv)
{
	...
	errorCase(68000000000 < CountMax); // XXX

	SosuMap = newBitList_A(CountMax / 2 + 100); // XXX
	...
	{
		if(pulseSec(1, NULL))
			execute_x(xcout("TITLE Prime2tox_memB - %I64u あたりまで計算済み...", count * count));
	...
	execute("TITLE Prime2tox_memB - 計算完了 出力中...");
	DispSosu();
