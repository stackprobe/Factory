...
> db.rec.tmp sqlcmd -S %1 -U %2 -P %3 -Q "select name from [sys].[databases]"
...
FOR /F %%D IN (db.tmp) DO (
	...
	> tbl.rec.tmp sqlcmd -S %1 -U %2 -P %3 -Q "select schema_id, name from [%%D].[sys].[tables]"
	> sch.rec.tmp sqlcmd -S %1 -U %2 -P %3 -Q "select schema_id, name from [%%D].[sys].[schemas]"
