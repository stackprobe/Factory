...
> tbl.rec.tmp sqlcmd -S %1 -Q "select schema_id, name from [%2].[sys].[tables]"
> sch.rec.tmp sqlcmd -S %1 -Q "select schema_id, name from [%2].[sys].[schemas]"
...
FOR /F %%T IN (tbl.tmp) DO (
	...
	rem bcp %%T out tbl.out.tmp -S %1 -T -c -t "\t"
	rem C:\Factory\SQLSvr200x\BcpReader.exe tbl.out.tmp tbl.out.tmp
