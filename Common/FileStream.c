...
HANDLE getWindowsHandleByFilePointer(FILE *fp) // ret: CreateFile() によって得られる HANDEL と同じ
{
...
{
	if(fwrite(directGetBuffer(block), 1, getSize(block), fp) != getSize(block)) // fwrite(, 1, 0, ) のとき 0 を返す。
	{
		error();
	}
}
