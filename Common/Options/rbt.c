...
{
	autoList_t *nodes = createAutoList(32);
...
/*
	direct
		...
		1: 右 -> 左 (反時計回り)
*/
static void Rotate(rbtTree_t *tree, rbtNode_t *node, uint direct)
{
...
void *rbtGetElement(rbtTree_t *tree, void *element) // element == NULL のとき LastAccessNode を返す。
{
	errorCase(!tree);
...
static uint GetNearestLeaf(rbtTree_t *tree, rbtNode_t *node, uint direct) // ret: direct 方向に Leaf が見つかれば 0 以外
{
	...
	return (uint)LastNode;
}
void rbtRemoveElement(rbtTree_t *tree, void *element) // element == NULL のとき LastAccessNode を削除する。
{
	rbtNode_t *node;
	rbtNode_t *p;
...
void rbtSeekForEndElement(rbtTree_t *tree, uint direct) // direct == 0: Left, 1: Right
{
	rbtNode_t *node;
	rbtNode_t *tmpNode;
...
void rbtSeekElement(rbtTree_t *tree, uint direct) // direct == 0: Left, 1: Right
{
	rbtNode_t *node;
	rbtNode_t *tmpNode;
